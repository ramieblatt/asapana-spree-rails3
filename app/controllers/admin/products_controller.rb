class Admin::ProductsController < Admin::ResourceController
  ...
  def collection
    ...
    unless request.xhr?
      ...
      @collection = @collection = @search.relation.select('DISTINCT products.*').paginate(pagination_options) # @search.relation.group_by_products_id.paginate(pagination_options)
    else
